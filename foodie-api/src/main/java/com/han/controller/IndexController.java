...
public class IndexController {
    ...
    public JSONResult carousel() {
        ...
        /**
         * 轮播图时会发生改变的, 什么时候Redis中的轮播图缓存失效 ?
         * (1)后台运营系统, 一旦广告（轮播）发生更改, 就可以删除缓存, 然后重置
         * (2)定时重置, 比如每天凌晨三点重置
    ...
    public JSONResult subCat(
        ...
        if(StringUtils.isBlank(catsStr)) {
            ...
            /**
             *  查询的key在redis中不存在, 对英国的id在数据库也不存在, 此时被非法用户进行攻击,
             *  大量的请求会直接打在db上, 造成宕机, 从而影响整个系统, 这种现象称之为缓存穿透。
             *  解决方案: 把空的数据也缓存起来(设置一个过期时间), 比如空字符串, 空对象, 空数组或list
             */
            if(list != null && list.size() > 0) {  //db中存在
